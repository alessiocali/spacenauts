...
public class MultiplayerSystem extends EntitySystem {
	...
	public MultiplayerSystem() {
		if (Spacenauts.getNetworkAdapter() == null) throw new GdxRuntimeException(new IllegalStateException("Can't start multiplayer system without a network adapter"));
	...
	public void update(float delta) {
		...
		if (na.getState() != AdapterState.GAME) {
			...
			if (na.getState() == AdapterState.FAILURE) Logger.log(LogLevel.ERROR, this.toString(),"Reason was: " + na.getFailureReason());
			...
			for (i = 0 ; i < MAX_HANDLED_MSG ; i++) {
				...
				if (friendDied) {
					...
					if (d != null) d.listeners.onDeath(friendPlayer);
					disconnect();
					return;
				} else if (friendDc) {
